...
type MeasurementsClient interface {
	...
	GetBandOverall(ctx context.Context, measurementID uuid.UUID, startFrequency, stopFrequency int64) (models.ModelNodeBandOverallResponse, error)
}
...
func (c *client) GetBandOverall(ctx context.Context, measurementID uuid.UUID, startFrequency, stopFrequency int64) (models.ModelNodeBandOverallResponse, error) {
	...
	var response models.ModelNodeBandOverallResponse
	if err := c.DoAndUnmarshal(ctx, request, &response); err != nil {
		return models.ModelNodeBandOverallResponse{}, fmt.Errorf("failed to get overall band value for measurement: %w", err)
	}
